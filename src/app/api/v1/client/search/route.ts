...
export async function POST(req: Request) {
  ...
  try {
    ...
    const [blogs, stores, categories, coupons, campaigns] = await Promise.all([
      ...
      CampaignModel.find({ title: searchFilter, product_status: "ACTIVE" }).limit(10).populate('store', 'name cashback_type cashback_rate store_link store_img').populate('category', 'name slug').select('store category offer_price calculated_cashback calculation_mode img_array product_tags  actual_price product_slug slug_type title  createAt updateAt _id').lean(),
    ]);
